...
There are 3 variables and 17568 observations.
...
# Graph using ggplot2
ggplot(steps_day, aes(x = steps)) + geom_histogram(fill = I("red"), alpha = I(0.5), 
    color = I("black")) + labs(title = "Histogram of Number of Steps per Day", 
    x = "Number of Steps", y = "Count") + geom_vline(xintercept = mean, colour = "green", 
    linetype = "longdash", size = 1) + geom_vline(xintercept = median, colour = "blue", 
    linetype = "longdash", size = 1) + annotate("text", x = mean, y = 7.5, label = name_mean) + 
...
# Graph using ggplot2
ggplot(steps_interval, aes(x = interval, y = steps)) + geom_line(color = "red", 
    size = 1) + labs(title = "Average of Steps for Interval", x = "Interval", 
    y = "Steps") + geom_vline(xintercept = interval_max, colour = "blue", linetype = "longdash", 
    size = 1) + annotate("text", x = interval_max, y = 200, label = name_interval)
```
...
The **835<sup>th</sup> interval** has the maximum activity on the average (average= 206.1698).
...
complete <- function(datos) {
    ...
    for (i in 1:n) {
        ...
        value <- steps_interval[steps_interval$interval == interval_vacios, 
            2]
...
ggplot(steps_day_complete, aes(x = steps)) + geom_histogram(fill = I("blue"), 
    alpha = I(0.5), color = I("black")) + labs(title = "New Histogram of Number of Steps per Day", 
    x = "Number of Steps", y = "Count") + geom_vline(xintercept = mean, colour = "green", 
    linetype = "longdash", size = 1) + geom_vline(xintercept = median, colour = "red", 
    linetype = "longdash", size = 1) + annotate("text", x = mean, y = 7.5, label = name_mean) + 
    annotate("text", x = median, y = 7, label = name_median)
```
...
# Plot
ggplot(data_weekday, aes(x = interval, y = steps)) + geom_line(color = "blue", 
    size = 1) + facet_wrap(~weekday, nrow = 2, ncol = 1) + labs(x = "Interval", 
    y = "Number of Steps")
```
